...
export default function App() {
  ...
  const handleFormSubmit = async (formData: any) => {
    ...
    if (companyName.length > 100) {
      alert(`Error: Company Name exceeds limit of 100 characters (actual: ${companyName.length}).`);
      return;
    }
    ...
    if (jobTitle.length > 100) {
      alert(`Error: Job Title exceeds limit of 100 characters (actual: ${jobTitle.length}).`);
      return;
    }

    if (location.length > 100) {
      alert(`Error: Location exceeds limit of 100 characters (actual: ${location.length}).`);
      return;
    }

    if (salaryRange.length > 50) {
      alert(`Error: Salary Range exceeds limit of 50 characters (actual: ${salaryRange.length}).`);
      return;
    }
    ...
    if (jobDescription.length > 5000) {
      alert(`Error: Job Description exceeds limit of 5000 characters (actual: ${jobDescription.length}).`);
      return;
    }

    if (notes.length > 1000) {
      alert(`Error: Personal Notes exceeds limit of 1000 characters (actual: ${notes.length}).`);
      return;
    }
